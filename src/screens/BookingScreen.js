...
const BookingScreen = ({ route, navigation }) => {
  ...
  // EXTREMELY safe parameter handling
  useEffect(() => {
    console.log('Route object:', route);
    console.log('Route params:', route?.params);
    
    if (route && route.params && route.params.hotel) {
      setHotel(route.params.hotel);
    } else {
      // Fallback to a default hotel
      setHotel({
        id: 1,
        name: 'Luxury Hotel',
      ...
      console.warn('No hotel params provided, using default hotel');
    }
  }, [route]);
  ...
  const handleConfirmBooking = async () => {
    if (!hotel) {
      Alert.alert('Error', 'Hotel information is not available');
      return;
    }
  ...
  // Show loading if hotel data isn't available yet
  if (!hotel) {
    return (
      <View style={globalStyles.container}>
        <View style={[globalStyles.screen, { justifyContent: 'center', alignItems: 'center' }]}>
          <Text style={[typography.body, { color: colors.textSecondary }]}>Loading booking information...</Text>
        </View>
      </View>
    );
  }
    ...
    <View style={globalStyles.container}>
      ...
      >
        <View style={globalStyles.screen}>
          ...
          {/* Confirm Button */}
          <TouchableOpacity
            style={[globalStyles.button, { marginTop: spacing.lg, marginBottom: spacing.xl }]}
